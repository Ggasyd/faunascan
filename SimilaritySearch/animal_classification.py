...
#fichier index contenant les représentations vectorielles de chaque audio(animals.index); ce fichier est au format index, la structure de fichier utilisé par Faiss
index_path="/Users/ariel/Downloads/animals.index"
index = faiss.read_index("/Users/ariel/Downloads/animals.index")

#fichier contenant le nom de chaque audio noms_animaux.txt
...
# Lire le contenu du fichier noms.animaux.txt et le convertir en liste
...
#Fonction de normalisation des noms pour que en fonction du nom de l'audio , le nom correspondant soit normalisé
...
# Définition de la fonction exponentielle négative utilisé pour nous retourner des probabilités à partir des distances.
def exp_negative(x):
    return math.exp(-x)
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python nom_fichier.py audio_path")
        sys.exit(1)

    audio_path = sys.argv[1]
    result = animal_classification(audio_path)
    print(result)
